...
%DP_APPLY_WORKSPACE Push the current app state into the base workspace and key model parameters.
%
% Collects parameters from guidata(fig) (physics, ICs, LQR, hysteresis, Ts, refs),
% exports them to the base workspace, and (when available) updates selected
% Simulink blocks (Integrator ICs and fixed-step solver Ts). Centralizes all
% workspace/model writes to avoid duplicated assignments across callbacks.
%
% Input:
