function dp_plot(fig, action)
    ...
    switch lower(action)
        case 'start'
            ...
            % awaryjny licznik czasu (gdy SimulationTime zwraca śmieci)
            try
        ...
        case 'tick'
            % TimerFcn nie może zabijać odświeżania wykresów.
            try
                st = '';
                try, st = get_param(a.mdl,'SimulationStatus'); catch, return; end

                % status zawsze aktualizujemy (także gdy paused/stopped)
                try
                ...
                % SimulationTime bywa "brudny" / pusty -> sscanf + fallback na wall time
                t = NaN;
                try
                ...
                % Guard against time going backwards after pause/continue glitches
                ...
                % x / theta muszą działać nawet jeśli u nie da się odczytać
                try
...
function s = mapStatusLocal(st)
    % zawsze zwracaj pojedynczy wiersz char (żeby uicontrol nie robił tekstu wieloliniowego)
    try
        st = lower(char(st));
    catch
