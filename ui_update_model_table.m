...
%UI_UPDATE_MODEL_TABLE Synchronize model parameters from the table into app.
%
% Table format: N-by-2 cell array
%   col1: label (e.g. 'Ts [s]')
%   col2: numeric value (or string convertible to numeric)
...
function key = labelToKey(label)
    ...
    try
        ...
        % remove units: take token before first space or '['
        s = strtrim(label);
